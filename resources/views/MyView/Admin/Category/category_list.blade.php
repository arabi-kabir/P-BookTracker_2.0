    ...
    <div class="card border border-secondary" style="margin-top: 50px;">
        <div class="card">
            <div class="card-header">
                <h4 class="m-0" style="font-weight: 300; display: inline-flex; padding-top: 6px;">Category List</h4>
                <button class="btn btn-purple waves-effect waves-light btn-sm float-right" data-toggle="modal" data-target=".bs-example-modal-center">Add new category</button>
            </div>
    ...
    <script>
        $(document).ready(function() {
            dataTable=  $('#datatable').DataTable({
                ...
                type:"POST",
                "ajax":{
                    "url": "{!! route('admin.category.list.getData') !!}",
                    "type": "POST",
                    data:function (d){
                        d._token="{{csrf_token()}}";
                    },
                ...
                columns:
                    [
...

                        {
                            "data": function(data)
                            {
                                return '<button class="btn btn-icon waves-effect waves-light btn-purple btn-sm mr-2" data-panel-id="'+data.category_id+'" onclick="edit_item(this)"><i class="fa fa-cog"></i></button>'+
                                       '<button class="btn btn-icon waves-effect waves-light btn-danger btn-sm" data-panel-id="'+data.category_id+'" onclick="delete_item(this)"><i class="fa fa-trash"></i></button>';
                            },
